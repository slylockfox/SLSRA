...
Serial.println(exc.readEncoderCount(8,1));
  exc.setMotorPower(ID,1,100);        // Set the power of motor 1 connnted to DC expansion controller with ID 1 to 50%
  ...
  exc.setMotorPower(ID,1,0);         // Set Motor 1 power to 0. (0 = coast stop; 125 = brake stop)
  delay(1000);                      // wait here 1 second

  exc.setMotorPower(ID,2,-100);       // Set the power of motor 2 connected to DC expansion controller with ID 1 to -50%.
  delay(2000);                      // wait here 2 seconds while motor spins
  exc.setMotorPower(ID,2,0);         // Set Motor 2 power to 0. (0 = coast stop; 125 = brake stop)
  delay(1000);                      // wait here 1 second.

  exc.setMotorPowers(ID,100,-100);     // Set the power of motor 1 to 50% and motor 2 to -50%. First number "1" is controller ID#.
  delay(2000);                      // wait here 2 seconds while motors spin
  exc.setMotorPowers(ID,125,125);    // Stop motors(0 = coast stop; 125 = brake stop)
